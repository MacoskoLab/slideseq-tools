...
# for i in $(seq 50);
# do echo '["qsub", "-l", "h_vmem=1m", "-pe", "smp", "1", "-binding", "linear:1", "-l", "h_rt=00:30:00", \
#           "-N", "sleep", "/broad/macosko/jlanglie/05_sysadmin/just_sleep.sh"]$$' > $RANDOM;
# done
...
def main():
    ...
    while True:
        ...
        out_stat = list(
            filter(
                lambda l: len(l) > 0 and not re.match("^(job-ID)|(---)", l), out_stat
            )
        )
        ...
        if len(out_ls) == 0:
            ...
            if (
                len(set(running_jnumbers) & set(all_submitted_jobs)) == 0
            ):  # TODOOO PUT BACK
                # if len(set(running_jnumbers)) == 0: # TODOOO PUT BACK
                print(
                    "AND NO JOBS WE'RE WAITING ON *{}".format(num_times_nothing_to_do)
                )
            ...
            if curr_num_jobs < MAX_NUM_JOBS:
                ...
                for fn in out_ls:
                    ...
                    with open(taskrunner_dir + fn) as f:
                        ...
                        if len(lines) < 1 or re.search(r"\$\$$", lines[0]) is None:
                            print(
                        ...
                        this_qsub = re.sub(r"\$\$$", "", this_qsub.strip())
...
# todo
# keep list of job ids and parse qstat. If all qstats are not in submission list ("" not in !) and has been 1/2 an hour,
# kill if files waiting, sleep 15-30. If no files, sleep 90 seconds add try catch so never stops on weird error
