    ...
    try:
        ...
        if not run_barcodematching:
            ...
            if len(email_address) > 1:
                subject = f"Slide-seq workflow finished for {flowcell_barcode}"
                content = (
                    f"The Slide-seq workflow for {library}_{locus_function_list} is finished."
                    " Please check the output folder for the results. Thank you for using the Slide-seq tools! "
                )
                call_args = [
                    "python",
        ...
        if len(email_address) > 1:
            ...
            content = (
                ...
                + " "
                + locus_function_list
                + " failed at the step of running specific analysis. Please check the log file for the issues. "
            )
            call_args = [
                "python",
                "{}/send_email.py".format(scripts_folder),
