...
parser = argparse.ArgumentParser(description='Submit flowcells to the Slide-seq flowcell alignment pipeline.')

parser.add_argument('flowcells', metavar='flowcells', type=str, nargs='+',
                    help='Names of flowcells to submit for processing (separated by spaces)')
parser.add_argument('--spreadsheet', dest='spreadsheet', metavar='spreadsheet',
                    default='1kwnKrkbl80LyE9lND0UZZJXipL4yfBbGjkTe6hcwJic', action='store',
                    help='Optional spreadsheet key to open and populate (default is for Macosko Slide-seq Flowcell Alignment)')
parser.add_argument('--worksheet_ind', dest='wks_ind', metavar='worksheet_ind', type=int,  
                    default=0, help='Which worksheet to open in spreadsheet (0-indexed) (default 0)')
parser.add_argument('--resubmit', dest='resubmit',  default=False,
                    action='store_true', help='Whether these flowcells are being resubmitted')
parser.add_argument('--dryrun', dest='dryrun',  default=False,
                    action='store_true', help='Show commands without running them')
...
for flowcell in flowcells:
	if '-' in flowcell and not args.diff_align:
		warnings.warn(('Flowcell name follows syntax for different secondary alignment;' +
			           ' please set --diff_align flag before running.').format(flowcell))
		continue
	flow_cells = wks.findall(flowcell)
	if len(np.unique([cell.col for cell in flow_cells])) > 1:
		warnings.warn(('Flowcell {} found in multiple columns;' +
			           ' not running until resolved.').format(flowcell))
		continue
	elif len(flow_cells) < 1:
		warnings.warn('Flowcell {} not found in spreadsheet; please add to sheet.'.format(flowcell))
		continue
	flow_rows = [cell.row for cell in flow_cells]
	
	# collect submission metadata
	submit_meta = [[cell.value for cell in wks.range(row, lib_i, row, gen_downsampling_i)] 
				   for row in flow_rows]
	submit_df = pd.DataFrame(submit_meta, 
	...
	if len(np.unique(submit_df.library)) != len(submit_df.library):
		warnings.warn(('Flowcell {} does not have unique library names; please fill out' +
			           ' naming metadata correctly or add suffixes.').format(flowcell), 
					  stacklevel = 2)
		continue
    
	# check for spaces
	if any([' ' in name for name in submit_df.library.values]):
		warnings.warn(('Flowcell {} has naming metadata containing spaces;' +
			           ' please remove all spaces before running.').format(flowcell), 
					  stacklevel = 2)
		continue

	if '' in submit_df.values:
		warnings.warn(('Flowcell {} does not have complete submission metadata (orange and blue cols);' +
			           ' please fill out before running.').format(flowcell), 
					  stacklevel = 2)
		continue

	# if bcl paths don't match
	if len(np.unique(submit_df.BCLPath)) > 1:
		warnings.warn(('Flowcell {} has multiple BCLPaths associated with it;' +
			           ' please correct to single path before running.').format(flowcell), 
					  stacklevel = 2)
		continue

	if not os.path.isdir(submit_df.BCLPath.values[0]):
		warnings.warn(('Flowcell {} has incorrect BCLPath associated with it;' +
			           ' please correct path before running.').format(flowcell), 
					  stacklevel = 2)
		continue

	if len(np.unique(submit_df.IlluminaPlatform)) > 1:
		warnings.warn(('Flowcell {} has multiple Illumina platforms associated with it;' +
			           ' please correct to single platform before running.').format(flowcell), 
					  stacklevel = 2)
		continue
	...
	if flowcell_platform not in platforms:
		warnings.warn(('Flowcell {} has unsupported platform; please correct to one of ' +
			           ' {} before running.').format(flowcell, ' '.join(platforms)), 
					  stacklevel = 2)
		continue
	...
	if not all(build_bool):
		warnings.warn(('Flowcell {} has reference which does not exist;' +
			           ' please correct reference values before running.').format(flowcell), 
					  stacklevel = 2)
		continue
	...
	if args.resubmit:
		...
		if not os.path.isdir(output_dir):
			warnings.warn(('Folder {} does not exist; please do not use' +
			               ' --resubmit flag for this flowcell.').format(output_dir), 
						  stacklevel = 2)
			continue
	else:	
		if os.path.isdir(output_dir):
			warnings.warn(('Folder {} exists; please remove the folder or use --resubmit flag.').format(output_dir), stacklevel = 2)
			continue
	...
	if not args.dryrun:
		...
		with open(manifest_file, 'w') as f:
			...
			f.write('picard_folder=/broad/macosko/bin/dropseq-tools/3rdParty/picard' + '\n')
			f.write('STAR_folder=/broad/macosko/bin/dropseq-tools/3rdParty/STAR-2.5.2a' + '\n')
