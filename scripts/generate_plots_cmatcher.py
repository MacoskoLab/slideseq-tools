...
def main():
    ...
    try:
        ...
        commandStr += f"OUTPUT={matched_shuffled_bam_file} SORT_ORDER=coordinate ASSUME_SORTED=true"
        for i in range(len(lanes)):
            if libraries[i] != library:
                continue
            for lane_slice in slice_id[lanes[i]]:
        ...
        commandStr += (
            "OUTPUT_HEADER=false UEI=" + library + " VALIDATION_STRINGENCY=SILENT"
        )
        ...
        os.system(
            "mv " + matched_bead_location_file2 + " " + matched_bead_location_file
        )
        ...
        commandStr += (
            "-jar "
            + picard_folder
            + "/picard.jar CollectRnaSeqMetrics TMP_DIR="
            + tmpdir
        )
        commandStr += (
        ...
        plt.ylabel(yl)
        plt.title(
            "Alignment quality for matched barcodes (FDR = " + str(ratio_UMIs) + ")"
        )
        plt.savefig(pp, format="pdf")
            ...
            if matched_dge_summary_barcodes[i] in matched_barcodes:
                matched_distances[
                    int(
                        distances[
                            np.where(
                                matched_barcodes == matched_dge_summary_barcodes[i]
                            )
                        ]
                    )
        ...
        pct_coding = np.loadtxt(
            f1, delimiter="\t", dtype="float", skiprows=7, usecols=18
        )
        ...
        if len(email_address) > 1:
            ...
            content = (
                f"The Slide-seq workflow for {library}_{reference2}"
                " is finished. Please check the output folder for the results. Thank you for using the Slide-seq tools!"
            )
            call_args = [
                "python",
        ...
        if len(email_address) > 1:
            ...
            content = (
                f"The Slide-seq workflow for {library} {reference2} "
                "failed at the step of generating plots for matched barcodes. Please check the log file for the issues."
            )
            call_args = [
                "python",
                "{}/send_email.py".format(scripts_folder),
