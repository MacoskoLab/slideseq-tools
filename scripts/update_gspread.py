...
for flowcell in flowcells:
	...
	for row in flow_rows:
		...
		alignment_folder = library_dir+'/'+wks.cell(row, date_i).value+'_'+library
		...
		for li in lists:
			...
			selected_cells = '{}/{}/alignment/{}.{}_transcripts_mq_{}_selected_cells.txt'.format(alignment_folder, reference2, library, min_transcripts_per_cell, base_quality)
			...
			file = '{}/{}/alignment/{}.ReadQualityMetrics.txt'.format(alignment_folder, reference2, library)
			if os.path.isfile(file):
				mat = np.loadtxt(file, delimiter='\t', dtype='int', skiprows=3, max_rows=1, usecols=(1))
				fraction_reads += str(mat * 100 // total_reads) + '%;'
            
			dge_summary = '{}/{}/alignment/{}.digital_expression_summary.txt'.format(alignment_folder, reference2, library)
			if os.path.isfile(dge_summary):
				dge_summary_reads = np.loadtxt(dge_summary, delimiter='\t', dtype='int', skiprows=7, usecols=(1))
				dge_summary_umi = np.loadtxt(dge_summary, delimiter='\t', dtype='int', skiprows=7, usecols=(2))
				dge_summary_gene = np.loadtxt(dge_summary, delimiter='\t', dtype='int', skiprows=7, usecols=(3))
