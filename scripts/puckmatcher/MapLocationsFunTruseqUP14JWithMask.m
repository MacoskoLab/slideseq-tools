function Illumina=MapLocationsFunTruseqUP14JWithMask(OutputDirectory,Bead,IlluminaBarcodes,PrimerNLigationSequence,PrimerUPLigationSequence,NumBases,SaveData,PuckName,varargin)
...
for seqnum=1:length(IlluminaBarcodes)
    ...
    [PrimerNcolorspace,badflagN]=bs2cs(seq(1:7),PrimerNLigationSequence,'T','');
...
%    [negIdentifiedBarcodes,negIA,negIB]=intersect(BeadBarcodes, NegBarcodes);
    ...
    %disp('Starting Illumina parfor loop')
       ...
       for ii = 1:length(UniqueBeadBarcodes)
           temp =  dec2base(UniqueBeadBarcodes(ii),5,NumBases);
           temp(Mask) = 'N';
            maskedtest{ii} = temp;
        end
    %for parnum=1:NumPar 
    ...
    for seq=1:nnz(localBase5Barcodes)
        ...
        temp = dec2base(localBase5Barcodes(seq),5,NumBases);
        temp(Mask) = 'N';
        maskedlocal = temp;
        hammingdistancetmp=cellfun(@(x) x==maskedlocal, maskedtest,'UniformOutput',false);
...
%        end
        ...
        temp= dec2base(localNegBarcodes(seq),5,NumBases);
        temp(Mask) = 'N';
        maskedlocalneg = temp;
        neghammingdistancetmp=cellfun(@(x) x==maskedlocalneg, maskedtest,'UniformOutput',false);
...
%    save([OutputDirectory,'hammingdistances.mat'],'hammingdistances');
    ...
    %print(figure(5),'-dpsc','-append',[OutputDirectory,'Report_',PuckName,'.ps']);
    print(figure(5),'-dpng',[OutputDirectory,'Report_',PuckName,'_5','.png']);
